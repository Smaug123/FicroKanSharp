...
module Recursive =
    ...
    let ``Recursive definitions, example 1`` () : unit =
        let justAte (t1 : TypedTerm<Entity>) (t2 : TypedTerm<Entity>) : Goal =
            Goal.disj
                (Goal.conj (TypedTerm.Goal.equiv t1 (TypedTerm.literal Entity.Mosquito)) (TypedTerm.Goal.equiv t2 (TypedTerm.Literal (Entity.Blood (TypedTerm.literal (Entity.Name (TypedTerm.literal "john")))))))
                (Goal.disj
                     (Goal.conj (TypedTerm.Goal.equiv t1 (TypedTerm.literal Entity.Frog)) (TypedTerm.Goal.equiv t2 (TypedTerm.literal Entity.Mosquito)))
                     (Goal.conj (TypedTerm.Goal.equiv t1 (TypedTerm.literal Entity.Stork)) (TypedTerm.Goal.equiv t2 (TypedTerm.literal Entity.Frog)))
                 )

        let rec isDigesting (t1 : TypedTerm<Entity>) (t2 : TypedTerm<Entity>) : Goal =
            Goal.disj
                (justAte t1 t2)
                (TypedTerm.Goal.callFresh (fun z -> Goal.delay (fun () -> Goal.conj (isDigesting t1 z) (isDigesting z t2))))
    ...
    let ``Recursive definitions, example 2`` () : unit =
        ...
        let child (t1 : TypedTerm<Human>) (t2 : TypedTerm<Human>) : Goal =
            children
            |> List.fold (fun state (parent, child) ->
